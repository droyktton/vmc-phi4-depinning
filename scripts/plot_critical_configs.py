...
from wall_io import read_wall_points, height_profile, parse_filename


def plot_one(ax, xs, ys, u, L, title=None, full_box=False):
...
def main():
    ...
    for path in files:
        ...
        samples.append((seed, hd, xs, ys, u))

        fig, ax = plt.subplots(figsize=(4, 4))
        title = f"seed {seed}, h_d = {hd:.6g}" if hd is not None else Path(path).name
        plot_one(ax, xs, ys, u, args.L, title=title, full_box=args.full_box)
        ax.set_xlabel("x")
        ax.set_ylabel("y")
    ...
    if args.grid:
        ...
        for ax, (seed, hd, xs, ys, u) in zip(axes, samples):
            plot_one(ax, xs, ys, u, args.L, title=f"seed {seed}", full_box=args.full_box)
            ax.set_xticks([])
            ax.set_yticks([])
        for ax in axes[n:]:
            ax.axis("off")
