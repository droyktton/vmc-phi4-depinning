...
do for[i=1:nconfs:1]{
	...
	#system("gawk 'NR<512 && NF==3 {print FNR,$2;}' zzz > www"); 
	system("gawk 'NR<512 && NF==3 {print $1,$2;}' zzz > www"); 

	fout=sprintf("%s.conf%d.dat",file,i);
	com=sprintf("mv www %s",fout); 
	system(com); 
	...
	#fout=sprintf("%s.%d.w2",file,i);	
	#archivo="www"; load "roughness_processor.gnu";
	#com=sprintf("mv procesado_www %s",fout); 
	#system(com);
